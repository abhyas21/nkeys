...
export default function AboutPage() {
  return (
    <div className="space-y-8">
      <section
        className="page-reveal rounded-[2rem] border border-stone-200 bg-white p-8 shadow-soft lg:p-10"
        style={{ "--delay": "40ms" }}
      >
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-stone-500">
          About
        </p>
        <h1 className="mt-2 text-4xl font-semibold text-ink">About NKeys</h1>
      ...
      <section className="page-reveal grid gap-6 md:grid-cols-3" style={{ "--delay": "120ms" }}>
        {aboutCards.map((card) => (
          <article
            key={card.title}
